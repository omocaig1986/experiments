...
WORKING_DIR = f"{BASE_DIR}/6rpi-2000req-th-10-blobs-k-4-pigo/6rpi-2000req-th-10-blobs-k-4-pigo-run-3"
N_MACHINES = 6
...
for i in range(len(dirs_list)):
    ...
    print("%d %.6f %.6f %.6f %.6f %d %d" % (i, pb_list[i], delays_list[i], probing_time_list[i], scheduling_time_list[i], accepted, rejected),
          file=out_file)
out_file.close()
...
# plot
plot([BLOB_SIZES[i] for i in range(len(dirs_list))], pb_list, "tau (ms)", "$P_B$", "multi_t_pb")
plot([BLOB_SIZES[i] for i in range(len(dirs_list))], delays_list, "tau (ms)", "Delay (s)", "multi_t_delay")
plot([BLOB_SIZES[i] for i in range(len(dirs_list))], probing_time_list, "tau (ms)", "Probing Time (s)", "multi_t_probing")
plot([BLOB_SIZES[i] for i in range(len(dirs_list))], scheduling_time_list, "tau (ms)", "Scheduling Time (s)", "multi_t_scheduling")
